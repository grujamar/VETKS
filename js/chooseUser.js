$(document).ready(function () {
        ...
        $.ajax({
            ...
            beforeSend: function () {
                ...
                .done(function (data) {
                    ...
                    $('#user-profile').append('<div class="row"><div class="col-md-4"><label class="font-weight-bold">Datum rodjenja</label></div><div class="col-md-8">' + (data.DateOfBirth).substring(8, 10) + '-' + (data.DateOfBirth).substring(5, 7) + '-' + (data.DateOfBirth).substring(0, 4) + '</div></div><hr>');
                    if (data.Status === true)
                    {
    ...
    function showCardInfoBasedOnLength(cardNumber, cardValidity, lasrUpdateTime, cardStatus) {
        if (cardStatus === true) {
            ...
            $('#card-profile').append('<div class="row"><div class="col-md-4"><label class="font-weight-bold">Broj kartice</label></div><div class="col-md-8">' + cardNumber + '</div></div><hr>');
            $('#card-profile').append('<div class="row"><div class="col-md-4"><label class="font-weight-bold">Rok važenja kartice</label></div><div class="col-md-8">' + (cardValidity).substring(8, 10) + '-' + (cardValidity).substring(5, 7) + '-' + (cardValidity).substring(0, 4) + '</div></div><hr>');
            $('#card-profile').append('<div class="row"><div class="col-md-4"><label class="font-weight-bold">Poslednji put ažurirana</label></div><div class="col-md-8">' + (lasrUpdateTime).substring(8, 10) + '-' + (lasrUpdateTime).substring(5, 7) + '-' + (lasrUpdateTime).substring(0, 4) + '</div></div><hr>');
